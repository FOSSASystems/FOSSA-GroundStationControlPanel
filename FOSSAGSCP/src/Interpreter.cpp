...
IGroundStationSerialMessage *Interpreter::Create_CMD_Retransmit(uint32_t senderId, char *message)
{
    ...
    IGroundStationSerialMessage* msg = this->Create_GroundStationSerialMessage(FCPI_FRAME_OP, CMD_RETRANSMIT, optDataLen, optData);

    delete[] optData;
    return msg;
}

IGroundStationSerialMessage *Interpreter::Create_CMD_Retransmit_Custom(uint8_t bandwidth, uint8_t spreadingFactor, uint8_t codingRate, uint16_t preambleSymbolLength, uint8_t crcEnabled, int8_t outputPower, uint32_t senderId, char *message)
{
    int messageLen = strlen(message) + 1;
    ...
    IGroundStationSerialMessage* msg = this->Create_GroundStationSerialMessage(FCPI_FRAME_OP, CMD_RETRANSMIT_CUSTOM, optDataLen, optData);
...
IGroundStationSerialMessage *Interpreter::Create_CMD_Transmit_System_Info()
{
    IGroundStationSerialMessage* msg = this->Create_GroundStationSerialMessage(FCPI_FRAME_OP, CMD_TRANSMIT_SYSTEM_INFO, 0, nullptr);
    return msg;
}

IGroundStationSerialMessage *Interpreter::Create_CMD_Get_Packet_Info()
{
    IGroundStationSerialMessage* msg = this->Create_GroundStationSerialMessage(FCPI_FRAME_OP, CMD_GET_PACKET_INFO, 0, nullptr);
    return msg;
}
...
IGroundStationSerialMessage *Interpreter::Create_CMD_Restart()
{
    IGroundStationSerialMessage* msg = this->Create_GroundStationSerialMessage(FCPI_FRAME_OP, CMD_RESTART, 0, nullptr);
    return msg;
}
...
IGroundStationSerialMessage *Interpreter::Create_CMD_Set_Callsign(char *callsign)
{
...
IGroundStationSerialMessage *Interpreter::Create_CMD_Set_SF_Mode(uint8_t spreadingFactor)
{
    char optData[1];
    optData[0] = spreadingFactor;

    IGroundStationSerialMessage* msg = this->Create_GroundStationSerialMessage(FCPI_FRAME_OP, CMD_SET_SF_MODE, 1, optData);
    return msg;
}

IGroundStationSerialMessage *Interpreter::Create_CMD_Set_MPPT_Mode(uint8_t temperatureSwitchEnabled, uint8_t keepAliveEnabled)
{
    char optData[2];
    optData[0] = temperatureSwitchEnabled;
    optData[1] = keepAliveEnabled;

    IGroundStationSerialMessage* msg = this->Create_GroundStationSerialMessage(FCPI_FRAME_OP, CMD_SET_MPPT_MODE, 2, optData);
    return msg;
}

IGroundStationSerialMessage *Interpreter::Create_CMD_Set_Low_Power_Mode_Enable(uint8_t lowPowerModeEnabled)
{
    char optData[1];
    optData[0] = lowPowerModeEnabled;

    IGroundStationSerialMessage* msg = this->Create_GroundStationSerialMessage(FCPI_FRAME_OP, CMD_SET_LOW_POWER_ENABLE, 1, optData);
    return msg;
}

IGroundStationSerialMessage *Interpreter::Create_CMD_Set_Receive_Windows(uint8_t fskReceiveWindowLength, uint8_t loraReceiveWindowLength)
{
    char optData[2];
    optData[0] = fskReceiveWindowLength;
    optData[1] = loraReceiveWindowLength;

    IGroundStationSerialMessage* msg = this->Create_GroundStationSerialMessage(FCPI_FRAME_OP, CMD_SET_RECEIVE_WINDOWS, 2, optData);
    return msg;
}
...
IGroundStationSerialMessage *Interpreter::Create_CMD_Set_Power_Limits(int16_t deploymentVoltageLimit, int16_t heaterVoltageLimit, int16_t cwBeepVoltageLimit, int16_t lowPowerVoltageLimit, float heaterTemperatureLimit, float mpptSwitchtemperatureLimit, uint8_t heaterDutyCycle)
{
    char optData[17];
    ...
    IGroundStationSerialMessage* msg = this->Create_GroundStationSerialMessage(FCPI_FRAME_OP, CMD_SET_POWER_LIMITS, 17, optData);
    return msg;
}

IGroundStationSerialMessage *Interpreter::Create_CMD_Set_RTC(uint8_t year, uint8_t month, uint8_t day, uint8_t dayOfWeek, uint8_t hours, uint8_t minutes, uint8_t seconds)
{
    char optData[7];
    optData[0] = year;
    optData[1] = month;
    ...
    IGroundStationSerialMessage* msg = this->Create_GroundStationSerialMessage(FCPI_FRAME_OP, CMD_SET_RTC, 7, optData);
    return msg;
}

IGroundStationSerialMessage *Interpreter::Create_CMD_Record_IMU(uint16_t sampleNumber, uint16_t samplingPeriod, uint8_t flags)
{
    char optData[5];
    ...
    IGroundStationSerialMessage* msg = this->Create_GroundStationSerialMessage(FCPI_FRAME_OP, CMD_RECORD_IMU, 5, optData);
    return msg;
}

IGroundStationSerialMessage *Interpreter::Create_CMD_Run_Manual_ACS(int8_t xHighBridgeMag, int8_t xLowBridgeMag, int8_t yHighBridgeMag, int8_t yLowBrightMag, int8_t zHBridgeHighMag, int8_t zBridgeLowMag, uint32_t xPulseLength, uint32_t yPulseLength, uint32_t zPulseLength, uint32_t maneuverDuration, uint8_t hbridgefaultFlags)
{
    char optData[23];
    ...
    IGroundStationSerialMessage* msg = this->Create_GroundStationSerialMessage(FCPI_FRAME_OP, CMD_RUN_MANUAL_ACS, 23, optData);
    return msg;
}

IGroundStationSerialMessage *Interpreter::Create_CMD_Log_GPS(uint32_t gpsLoggingDuration, uint32_t gpsLoggingStartOffset)
{
    char optData[8];
    ...
    IGroundStationSerialMessage* msg = this->Create_GroundStationSerialMessage(FCPI_FRAME_OP, CMD_LOG_GPS, 8, optData);
    return msg;
}

IGroundStationSerialMessage *Interpreter::Create_CMD_Get_GPS_Log(uint8_t newestEntriesFirst, uint16_t gpsLogOffsetAsNMEASeq, uint16_t NMEASentencesToDownlink)
{
    char optData[5];
    ...
    IGroundStationSerialMessage* msg = this->Create_GroundStationSerialMessage(FCPI_FRAME_OP, CMD_GET_GPS_LOG, 5, optData);
    return msg;
}

IGroundStationSerialMessage *Interpreter::Create_CMD_Get_Flash_Contents(uint32_t flashAddress, uint8_t numBytes)
{
    char optData[5];
    ...
    IGroundStationSerialMessage* msg = this->Create_GroundStationSerialMessage(FCPI_FRAME_OP, CMD_GET_FLASH_CONTENTS, 5, optData);
    return msg;
}

IGroundStationSerialMessage *Interpreter::Create_CMD_Get_Picture_Length(uint8_t pictureSlot)
{
    char optData[1];
    optData[0] = pictureSlot;

    IGroundStationSerialMessage* msg = this->Create_GroundStationSerialMessage(FCPI_FRAME_OP, CMD_GET_PICTURE_LENGTH, 1, optData);
    return msg;
}
...
IGroundStationSerialMessage *Interpreter::Create_CMD_Route(char *fcpFrame)
{
    ...
    IGroundStationSerialMessage* msg = this->Create_GroundStationSerialMessage(FCPI_FRAME_OP, CMD_ROUTE, messageLen, optData);
...
IGroundStationSerialMessage *Interpreter::Create_CMD_Set_Flash_Contents(uint32_t flashAddress, char *data)
{
    ...
    IGroundStationSerialMessage* msg = this->Create_GroundStationSerialMessage(FCPI_FRAME_OP, CMD_SET_FLASH_CONTENTS, messageLen, optData);
...
IGroundStationSerialMessage *Interpreter::Create_CMD_Set_TLE(char *tle)
{
    ...
    IGroundStationSerialMessage* msg = this->Create_GroundStationSerialMessage(FCPI_FRAME_OP, CMD_SET_TLE, optDataLen, optData);

    delete[] optData;
    return msg;
}

IGroundStationSerialMessage *Interpreter::Create_CMD_Run_GPS_Command(char *skyTraqBinaryProtocolMessage)
{
    ...
    IGroundStationSerialMessage* msg = this->Create_GroundStationSerialMessage(FCPI_FRAME_OP, CMD_RUN_GPS_COMMAND, optDataLen, optData);
...
IGroundStationSerialMessage *Interpreter::Create_CMD_Set_Sleep_Intervals(std::vector<sleep_interval_t> sleepIntervals)
{
    ...
    IGroundStationSerialMessage* msg = this->Create_GroundStationSerialMessage(FCPI_FRAME_OP, CMD_SET_SLEEP_INTERVALS, optDataLen, optData);
...
IGroundStationSerialMessage *Interpreter::Create_CMD_Abort()
{
    IGroundStationSerialMessage* msg = this->Create_GroundStationSerialMessage(FCPI_FRAME_OP, CMD_ABORT, 0, nullptr);
    return msg;
}

IGroundStationSerialMessage *Interpreter::Create_CMD_Maneuver(uint8_t controlFlags, uint32_t maneuverLength)
{
    char optData[5];
...
IGroundStationSerialMessage *Interpreter::Create_CMD_Set_ADCS_Parameters(float maximumPulseIntensity, float maximumPulseLength, float detumblingAngularVelocity, float minimumIntertialMoment, float pulseAmplitude, float calculationTolerance, float activeControlEulerAngleChangeTolerance, float activeControlAngularVelocityChangeTolerance, float eclipseThreshold, float rotationMatrixWeightRatio, float rotationVerificationTriggerLevel, float kalmanFilterDisturbanceCovariance, float kalmanFilterNoiseCovariance, uint32_t adcsUpdateTimeStepPeriod, uint32_t hbridgeTimerUpdatePeriod, int8_t hbridgeValueHighOutput, int8_t hbridgeValueLowOutput, uint8_t numControllers)
{
    ...
    IGroundStationSerialMessage* msg = this->Create_GroundStationSerialMessage(FCPI_FRAME_OP, CMD_SET_ADCS_PARAMETERS, 63, optData);
    return msg;
}

IGroundStationSerialMessage *Interpreter::Create_CMD_Erase_Flash(uint32_t sectorAddress)
{
    char optData[4];
    ...
    IGroundStationSerialMessage* msg = this->Create_GroundStationSerialMessage(FCPI_FRAME_OP, CMD_ERASE_FLASH, 4, optData);
    return msg;
}
...
IGroundStationSerialMessage *Interpreter::Create_CMD_Detumble(uint8_t flags, uint32_t length)
{
    char optData[5];
    optData[0] = flags;
    optData[1] = length;
    ...
    IGroundStationSerialMessage* msg = this->Create_GroundStationSerialMessage(FCPI_FRAME_OP, CMD_DETUMBLE, 5, optData);
    return msg;
}

IGroundStationSerialMessage *Interpreter::Create_CMD_Set_IMU_Offset(uint8_t xAxisGyroOffset, uint8_t yAxisGyroOffset, uint8_t zAxisGyroOffset, uint8_t xAxisAcceleOffset, uint8_t yAxisAcceleOffset, uint8_t zAxisAcceleOffset, uint8_t xAxisMagneticOffset, uint8_t yAxisMagneticOffset, uint8_t zAxisMagneticOffset)
{
    ...
    IGroundStationSerialMessage* msg = this->Create_GroundStationSerialMessage(FCPI_FRAME_OP, CMD_SET_IMU_OFFSET, 36, optData);
    return msg;
}

IGroundStationSerialMessage *Interpreter::Create_CMD_Set_IMU_Calibration(float transformationMatrix[9], float biasVector[3])
{
    ...
    IGroundStationSerialMessage* msg = this->Create_GroundStationSerialMessage(FCPI_FRAME_OP, CMD_SET_IMU_CALIBRATION, 47, optData);
    return msg;
}
...
IGroundStationSerialMessage *Interpreter::Create_CMD_Store_And_Forward_Add(uint32_t messageId, char *message)
{
    int messageLen = strlen(message) + 1;
    ...
    IGroundStationSerialMessage* msg = this->Create_GroundStationSerialMessage(FCPI_FRAME_OP, CMD_STORE_AND_FORWARD_ADD, optDataLen, optData);
...
IGroundStationSerialMessage *Interpreter::Create_CMD_Store_And_Forward_Request(uint32_t messageId)
{
    char optData[4];
    optData[0] = messageId;
    optData[1] = messageId >> 8;
    ...
    IGroundStationSerialMessage* msg = this->Create_GroundStationSerialMessage(FCPI_FRAME_OP, CMD_STORE_AND_FORWARD_REQUEST, 4, optData);
    return msg;
}

IGroundStationSerialMessage *Interpreter::Create_CMD_Request_Public_Picture(uint8_t pictureSlot, uint16_t picturePacketId)
{
    ...
    IGroundStationSerialMessage* msg = this->Create_GroundStationSerialMessage(FCPI_FRAME_OP, CMD_REQUEST_PUBLIC_PICTURE, 3, optData);
    return msg;
}
